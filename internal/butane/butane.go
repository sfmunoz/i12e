...
func (b *Butane) butaneRender() (*bytes.Buffer, error) {
	...
	cmd := exec.Command("./scripts/sops.sh", "rclone-conf")
	if b.cfg.Env == config.EnvProd {
	...
	if err != nil {
		return nil, fmt.Errorf("'./scripts/sops.sh rclone-conf' failed: err=%s; buf_err=%s", err, be)
	}
