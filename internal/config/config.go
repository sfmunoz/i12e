...
type Config struct {
	...
	} `mapstructure:"i12e" validate:"i12e_required=artifact:butane:server"`
	K3s *struct {
	...
	} `mapstructure:"k3s" validate:"i12e_required=artifact:butane:server"`
	Rclone *struct {
		Remote string `mapstructure:"remote" validate:"gte=1"`
	} `mapstructure:"rclone" validate:"i12e_required=artifact:butane:server"`
	Artifact *struct {
		PortKnocking []int `mapstructure:"port_knocking" validate:"len=4"` // valid: 4, see https://github.com/sfmunoz/i12e/issues/138
	} `mapstructure:"artifact" validate:"i12e_required=artifact:butane:server"`
	KubeVip *struct {
	...
	} `mapstructure:"kube_vip" validate:"i12e_required=artifact:butane:server"`
	Mesh *struct {
	...
	} `mapstructure:"mesh" validate:"i12e_required=artifact:butane:server"`
	Pushover *struct {
		UserKey string `mapstructure:"user_key" validate:"gte=1"`
		Token   string `mapstructure:"token" validate:"gte=1"`
	} `mapstructure:"pushover" validate:"i12e_required=artifact:butane:server"`
	Butane *struct {
	...
	} `mapstructure:"butane" validate:"i12e_required=artifact:butane:server"`
	Server *struct {
	...
	} `mapstructure:"server" validate:"i12e_required=artifact:butane:server"`
}
